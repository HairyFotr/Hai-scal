...
public class FrmPrintXML implements AbsVisitor {
	...
	public void visit(AbsProgram acceptor) {
		...
		{
			FrmFrame ff = FrmDesc.getFrame(acceptor);
			if(ff!=null) ff.toXML(xml); else xml.print("<abserror kind=\"NullFrame\"/>\n");
		}
		/*{
			FrmDesc.getFrame(acceptor).toXML(xml);
		}*/
	...
	public void visit(AbsVarDecl acceptor) {
		...
		{
			FrmAccess fa = FrmDesc.getAccess(acceptor);
			if(fa!=null) fa.toXML(xml); else xml.print("<abserror kind=\"NullAccess\"/>\n");
		}
		acceptor.name.accept(this);
		acceptor.type.accept(this);
		xml.print("</absnode>\n");
	...
	public void visit(AbsRepeatStmt acceptor) {
		if (acceptor.error) { xml.println("<abserror kind=\"RepeatStmt\"/>"); return; }
		xml.print("<absnode " + printPos(acceptor) + " kind=\"RepeatStmt\">");
		acceptor.cond.accept(this);
		acceptor.stmts.accept(this);
		xml.print("</absnode>\n");
	}
	...
	public void visit(AbsReturn acceptor) {
		if (acceptor.error) { xml.println("<abserror kind=\"Return\"/>"); return; }
		xml.print("<absnode " + printPos(acceptor) + " kind=\"Return\">");
		if(acceptor.expr != null) acceptor.expr.accept(this);
		xml.print("</absnode>\n");
	}
}
